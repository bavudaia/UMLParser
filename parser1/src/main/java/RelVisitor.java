...
public class RelVisitor extends GenericVisitorAdapter<String, Object>{
	...
	public static List<HasRel> hasRelList = null;
	
	static{
		hasRelList = new ArrayList<HasRel>();
	}
				...
				for(String typeParam : typeParams)
				{
					...
					for(String classVar : myClassVarList)
					{
						...
						if(typeParam!= null && typeParam.equals(classNameFromUML)){
							...
							int index =Utils.searchHasList(hasRelList, currentClassOrInterfaceName, typeParam);
							/*unidirectional*/
							if(index  == -1){
								HasRel currRel = new HasRel();
								currRel.fromClass = currentClassOrInterfaceName;
								currRel.toClass = typeParam;
								currRel.relMag = many==0?"1":"*";
								hasRelList.add(currRel);
							}
							/*reverse direction*/
							else
							{
								hasRelList.get(index).isBidirectional = true;
								hasRelList.get(index).revRelMag = many==0?"1":"*";
							}							
						...
						for(String interfaceVar : myInterfaceVarList)
						{
							...
							if(typeParam!= null && typeParam.equals(interfaceNameFromUML)){								
								...
								if(index  == -1){
									HasRel currRel = new HasRel();
									currRel.fromClass = currentClassOrInterfaceName;
									currRel.toClass = Constants.interfaceBegin + typeParam;
									currRel.relMag = many==0?"1":"*";
									hasRelList.add(currRel);
								}
								/*reverse direction*/
